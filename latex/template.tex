...
\data{2026}
\orientador{Prof. Dr. Pedro Augusto Queiroz de Assis} %TCC
...
\begin{folhadeaprovacao}
  ...
  \begin{center}
   ...
   \assinatura{\textbf{Prof. Dr. Fernando Lourenço de Souza}}% \\ Convidado 1} %TCC:
   \assinatura{\textbf{Dr. Arthur Pereira Borges}}% \\ Convidado 2} %TCC:
...
Gostaria de agradecer à Universidade Federal de Uberlândia pela oportunidade de realizar minha formação em uma instituição de excelência. Agradeço também à Faculdade de Engenharia Mecânica e ao Curso de Engenharia Mecatrônica, pela infraestrutura disponibilizada, pelos recursos oferecidos e pelo corpo docente altamente qualificado, que desempenhou papel essencial em minha trajetória universitária. 

Agradeço especialmente ao Prof. Dr. Pedro Augusto Queiroz Assis, não apenas pela paciência e dedicação na orientação deste trabalho, mas também por sua atuação exemplar como professor. Seu profissionalismo e comprometimento com o ensino certamente se tornaram uma grande referência para mim.

Expresso minha profunda gratidão à minha família, por todo o apoio e carinho ao longo dessa jornada. Em especial, aos meus pais, Enedina e Edilson, que sempre foram e continuam sendo minha base para tudo. Essa trajetória definitivamente não foi fácil, mas só foi possível graças aos esforços, à confiança e ao amor de vocês.

Sou grato também a todos os meus amigos, que tornaram essa caminhada mais leve. Cada conversa, momento de descontração e incentivo foi extremamente importante. Em especial, agradeço a Jorge, Nicholas e Sofia, que estiveram presentes de forma marcante ao longo dessa etapa.

Por fim, deixo meu agradecimento à EDROM e e a todos os colegas da equipe. Foi por meio dessa experiência que passei a me identificar de forma mais concreta com o curso e com a carreira que desejo seguir, além de encontrar um ótimo ambiente de aprendizado.

A todos que, de alguma forma, contribuíram para esta conquista, deixo meu mais sincero muito obrigado!
...
O presente trabalho aborda o problema crônico das perdas de inventário por furtos no setor varejista, com o objetivo de comparar empiricamente o desempenho de duas conhecidas arquiteturas de aprendizado profundo na identificação automatizada de comportamentos suspeitos em vídeos: uma Rede Neural Convolucional Tridimensional (I3D) e um Transformador de Visão (TimeSFormer). Para a condução do estudo, a metodologia englobou a construção de um banco de dados próprio através da agregação, filtragem e segmentação de amostras de bases públicas, submetendo os modelos a dezesseis configurações experimentais. Tais experimentos variaram a estratégia de ajuste de pesos, a resolução temporal e o uso de imagens de fluxo óptico combinadas ao padrão de cores convencional. A avaliação incluiu ainda uma simulação de monitoramento contínuo em vídeos reais não editados, utilizando uma abordagem de inferência por janela deslizante. Os resultados demonstram que o ajuste profundo de todos os parâmetros da rede configura uma etapa estritamente essencial para a convergência e o aprendizado em ambas as arquiteturas. O Transformador de Visão alcançou a melhor capacidade de generalização e controle de alarmes falsos no conjunto de teste, mas exige um investimento computacional substancialmente maior com elevado tempo de inferência. Em contrapartida, a Rede Neural Convolucional apresentou um comportamento preditivo conservador e com maior tendência a falsos positivos, destacando-se, no entanto, pela alta eficiência e baixa latência em sua execução. Conclui-se que a aplicação mercadológica exige um compromisso técnico direto entre a alta seletividade de modelos baseados em atenção e a escalabilidade e rapidez das redes convolucionais, além de constatar que ambos os sistemas compartilham vulnerabilidades críticas e falhas de detecção frente a oclusões severas e movimentos sutis no ambiente comercial.
...
\item[SL] \textit{Supervised Learning}
\item[RGB] \textit{Red, Green, Blue}
\item[I3D] \textit{Inflated 3D ConvNet}
\item[K400] \textit{Kinetics-400}
\item[SSv2] \textit{Something-Something V2}
\item[BCE] \textit{Binary Cross-Entropy}
\item[CEL] \textit{Cross-Entropy Loss}
...
\item [AdamW] \textit{Adam with Decoupled Weight Decay}
\end{siglas}
% ---
...
Na Avaliação em Vídeos Reais, apresenta-se a aplicação dos melhores modelos de cada arquitetura em um cenário prático de monitoramento contínuo de fluxo de vídeo, utilizando uma abordagem de inferência por janela deslizante. O desempenho prático é avaliado sobre um conjunto inédito de sete vídeos reais de ocorrências variadas no varejo. A análise revela que o TimeSFormer demonstra maior seletividade e capacidade de sincronização com o exato momento da ocultação , embora sofra com latência em alguns casos. Em contrapartida, o modelo I3D destaca-se na detecção de ações abruptas , mas apresenta uma forte tendência à saturação de probabilidade e falsos positivos gerados pela manipulação legítima de produtos. Por fim, evidencia-se que ambas as arquiteturas compartilham uma limitação crítica: a falha na detecção diante de oclusões severas e movimentos corporais sutis.

Na Avaliação de Implementação em Cenário Real, apresenta-se um estudo de viabilidade técnica e financeira para a implantação do sistema proposto em um ambiente varejista padrão. O capítulo detalha o dimensionamento da infraestrutura de captação de imagens e a modelagem da demanda computacional para inferência contínua, transpondo o custo de processamento do modelo TimeSFormer para requisitos reais de \textit{hardware} (GPUs). Por fim, realiza-se uma projeção econômica que contrapõe os custos estimados de aquisição dos equipamentos à perspectiva de redução de perdas financeiras geradas por furtos, demonstrando o potencial de retorno sobre o investimento e a viabilidade prática da solução.

Por fim, em Conclusões, sintetizam-se os achados empíricos da pesquisa, destacando que o ajuste profundo de todos os pesos da rede foi estritamente essencial para a convergência e o aprendizado eficaz das dinâmicas de furto em ambas as arquiteturas. Confirma-se a superioridade do modelo TimeSFormer na generalização para dados inéditos, alcançando 100\% de precisão e controle superior na redução de alarmes falsos, em contraste com o comportamento conservador e a tendência a falsos positivos do modelo convolucional I3D. Discute-se também o compromisso técnico inerente à aplicação mercadológica, evidenciando que a maior precisão do TimeSFormer exige um investimento computacional substancialmente maior, ao passo que o I3D se destaca pela alta eficiência e baixa latência. O capítulo encerra o trabalho apontando as vulnerabilidades compartilhadas por ambos os modelos frente a oclusões severas e movimentos corporais sutis, além de propor diretrizes para trabalhos futuros, tais como a ampliação do banco de dados com filmagens locais do varejo e a integração com abordagens multimodais baseadas em estimativa de pose esqueletal.
...
\begin{figure}[htbp]
    ...
    \includegraphics[height=0.60\textheight]{figuras/timesformer_training_loss.png}
...
\midrule
I3D - Completo / RGB (64 quadros) & 12,57 & 79,52 \\
...
Os experimentos apresentados no capítulo anterior foram realizados exclusivamente com amostras do conjunto de dados agregado. Embora esse procedimento permita o treinamento e a comparação controlada entre arquiteturas, as condições típicas de monitoramento reais não são reproduzidas, pois - na prática - o modelo processa um fluxo contínuo de vídeo com duração indefinida. É importante ressaltar que esse tipo de vídeo real não foi utilizado na etapa de treinamento, o que configura o cenário atual como um caso extremo de avaliação para a capacidade de generalização dos modelos treinados. Nesse contexto, a avaliação demanda uma estratégia de inferência diferente da utilizada no treinamento, em que cada entrada corresponde a um clipe de duração fixa com rótulo único. Para lidar com essa diferença, adota-se uma abordagem baseada em janela deslizante, com agregação temporal por quadro, descrita a seguir.

\section[Metodologia de Inferência]{Metodologia de Inferência}

Os vídeos de monitoramento real são primeiramente reamostrados com uma taxa uniforme de 25 quadros por segundo, taxa adotada durante o pré-processamento do conjunto de treinamento. Cada quadro é então redimensionado com preservação da proporção, o maior lado é escalado para 224 pixels e recortado centralmente para a resolução de entrada padrão de ambos os modelos $224 \times 224$ pixels.

Sobre a sequência de quadros reamostrada aplica-se uma janela deslizante de tamanho fixo e passo $s = 8$ quadros. O tamanho da janela é mantido idêntico ao usado durante o treinamento de cada modelo: $W_{\text{I3D}} = 64$ quadros e $W_{\text{TS}} = 32$ quadros. A cada posição de janela, a subsequência extraída é encaminhada ao respectivo modelo, que produz uma probabilidade escalar $p \in [0, 1]$ associada à classe \textit{Shoplifting}.


Em razão do passo $s < W$, janelas consecutivas se sobrepõem de forma que um quadro de índice $f$ pertence a mais de uma janela. Em vez de reportar predições por janela, que introduziriam descontinuidades temporais, opta-se por agregar as predições ao nível do quadro. A probabilidade atribuída ao quadro $f$ é a média aritmética das predições de todas as janelas que o contêm, como na \autoref{fig:janela_deslizante}, o resultado é uma curva de probabilidade contínua e de resolução temporal unitária, alinhada quadro a quadro com o vídeo original reamostrado. Um quadro é classificado como suspeito quando $\hat{p}(f)$ excede o limiar de decisão calibrado no conjunto de validação para cada modelo. Para o modelo I3D, utilizou-se o limiar que otimiza o desempenho global no conjunto de validação, resultando em $\tau_{\text{I3D}} = 0{,}7449$. Para o TimeSFormer, o limiar corresponde à fronteira natural do \textit{softmax} binário, $\tau_{\text{TS}} = 0{,}50$.

\begin{figure}[!htbp]
    \centering
    \caption[Exemplo de aplicação da abordagem de janela deslizante sobre vídeo real.]{Exemplo de aplicação da abordagem de janela deslizante sobre vídeo real.}
	\includegraphics[width=0.8\textwidth]{figuras/janela_desliz.png}
	\legend{Fonte - Do autor.}
    \label{fig:janela_deslizante}
\end{figure}

\section[Conjunto de Vídeos Reais]{Conjunto de Vídeos Reais}

Foram coletados sete vídeos reais de furtos em estabelecimentos varejistas, sem qualquer uso nas etapas de treinamento ou avaliação anteriores. Esses vídeos, obtidos de fontes públicas na internet, contemplam diferentes tipos de ocorrência em ambientes distintos. As principais características dos vídeos utilizados nesse capítulo encontram-se na \autoref{tab:real_videos}.

\begin{table}[htbp]
\centering
\caption{Características dos vídeos reais de furto utilizados na avaliação.}
\label{tab:real_videos}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllll}
\toprule
\textbf{Nome} & \textbf{Duração total (s)} & \textbf{Ambiente} & \textbf{Método de ocultação} \\
\midrule
furto-celular & 8 & Salão de beleza & Sem ocultação           \\
furto-pc & 29 & Loja de roupas     & Mochila  \\
furto-roupa & 33 & Loja de Roupas           & Bolsa             \\
furto-supermercado & 16 & Supermercado & Bolsa           \\
furto-shopping & 18 & Shopping       & Bolso             \\
furto-tenis1 & 9 & Loja de Calçados       & Bolsa  \\
furto-tenis2 & 7 & Loja de Calçados       & Bolsa  \\
\bottomrule
\end{tabular}%
}
\end{table}
...
A análise das curvas de probabilidade geradas pela inferência em janela deslizante revela comportamentos distintos entre as duas arquiteturas. Enquanto o modelo I3D apresenta uma curva frequentemente saturada em patamares elevados, o TimeSFormer demonstra maior seletividade, reagindo de forma mais sensível aos eventos de interesse. Para ilustrar essas dinâmicas e justificar as classificações, os gráficos contínuos foram analisados em conjunto com os quadros originais dos vídeos, permitindo a verificação de como transformações visuais específicas na cena impactaram a confiança de cada modelo.

\subsection[Casos de Sucesso]{Casos de Sucesso}

Os casos de sucesso foram definidos pela capacidade do modelo em elevar a probabilidade de furto acima do limiar estabelecido sincronizadamente com a ação real de ocultação ou subtração.

O modelo TimeSFormer obteve sucesso notável nos vídeos \textit{furto-supermercado}, \textit{furto-tenis1} e \textit{furto-tenis2}. Conforme ilustrado nas \autoref{fig:furto-supermercado-ts}, \autoref{fig:furto-tenis1-ts} e \autoref{fig:furto-tenis2-ts}, a curva de probabilidade do modelo mantém-se em níveis basais durante a fase de aproximação e interação normal do cliente com o produto. O pico da curva, que cruza o limiar de detecção, coincide com os quadros em que ocorre a ocultação efetiva do item, retornando à normalidade logo após a conclusão do movimento.

\begin{figure}[!htbp]
    \centering
    \caption[Curva de probabilidade do modelo TimeSFormer para o vídeo \textit{furto-supermercado}.]{Curva de probabilidade do modelo TimeSFormer para o vídeo \textit{furto-supermercado}.}
	\includegraphics[width=1.0\textwidth]{figuras/furto-supermercado-ts.png}
	\legend{Fonte - Do autor.}
    \label{fig:furto-supermercado-ts}
\end{figure}
\begin{figure}[!htbp]
    \centering
    \caption[Curva de probabilidade do modelo TimeSFormer para o vídeo \textit{furto-tenis1}.]{Curva de probabilidade do modelo TimeSFormer para o vídeo \textit{furto-tenis1}.}
	\includegraphics[width=1.0\textwidth]{figuras/furto-tenis1-ts.png}
	\legend{Fonte - Do autor.}
    \label{fig:furto-tenis1-ts}
\end{figure}
\begin{figure}[!htbp]
    \centering
    \caption[Curva de probabilidade do modelo TimeSFormer para o vídeo \textit{furto-tenis2}.]{Curva de probabilidade do modelo TimeSFormer para o vídeo \textit{furto-tenis2}.}
	\includegraphics[width=1.0\textwidth]{figuras/furto-tenis2-ts.png}
	\legend{Fonte - Do autor.}
    \label{fig:furto-tenis2-ts}
\end{figure}

Por outro lado, o modelo I3D demonstrou um sucesso expressivo no vídeo \textit{furto-celular}. Neste cenário, a arquitetura apresentou alta precisão temporal, conforme demonstrado na \autoref{fig:furto-celular-i3d}. A avaliação quadro a quadro revela que o modelo classifica a sequência como suspeita estritamente no instante em que o indivíduo retira o aparelho da bancada de forma abrupta. Adicionalmente, no vídeo \textit{furto-pc}, o I3D obteve um sucesso parcial: embora tenha conseguido identificar um pico claro de probabilidade durante o ato do furto, o modelo manteve uma probabilidade residual persistentemente alta em trechos do vídeo onde nenhuma ação anômala ocorria, demonstrado na \autoref{fig:furto-pc-i3d}.

\begin{figure}[!htbp]
    \centering
    \caption[Curva de probabilidade do modelo I3D para o vídeo \textit{furto-celular}.]{Curva de probabilidade do modelo I3D para o vídeo \textit{furto-celular}.}
	\includegraphics[width=1.0\textwidth]{figuras/furto-celular-i3d.png}
	\legend{Fonte - Do autor.}
    \label{fig:furto-celular-i3d}
\end{figure}
\begin{figure}[!htbp]
    \centering
    \caption[Curva de probabilidade do modelo I3D para o vídeo \textit{furto-pc}.]{Curva de probabilidade do modelo I3D para o vídeo \textit{furto-pc}.}
	\includegraphics[width=1.0\textwidth]{figuras/furto-pc-i3d.png}
	\legend{Fonte - Do autor.}
    \label{fig:furto-pc-i3d}
\end{figure}

\subsection[Casos de Falha]{Casos de Falha}

As falhas observadas evidenciam as limitações de generalização dos modelos frente a oclusões severas e ações de baixa magnitude visual. Os vídeos \textit{furto-roupa} e \textit{furto-shopping} representaram falhas totais para ambas as arquiteturas, com as probabilidades mantendo-se significativamente abaixo dos limiares de alarme durante todo o clipe. A análise visual desses casos sugere que o método de ocultação, como nas \autoref{fig:trechos-roupa} e \autoref{fig:trechos-shopping}, inibiu de forma contundente a extração das características espaço-temporais necessárias para a discriminação do ilícito.

\begin{figure}[!htbp]
    \centering
    \caption[Curva de probabilidade dos modelo I3D e TimeSFormer para o vídeo \textit{furto-roupa}.]{Curva de probabilidade dos modelo I3D e TimeSFormer para o vídeo \textit{furto-roupa}.}
	\includegraphics[height=0.38\textheight]{figuras/trechos-roupa.png}
	\legend{Fonte - Do autor.}
    \label{fig:trechos-roupa}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \caption[Curva de probabilidade dos modelo I3D e TimeSFormer para o vídeo \textit{furto-shopping}.]{Curva de probabilidade dos modelo I3D e TimeSFormer para o vídeo \textit{furto-shopping}.}
	\includegraphics[height=0.38\textheight]{figuras/trechos-shopping.png}
	\legend{Fonte - Do autor.}
    \label{fig:trechos-shopping}
\end{figure}

O I3D exibiu uma clara tendência a falsos positivos e à total falta de precisão temporal nos vídeos \textit{furto-tenis1}, \textit{furto-tenis2} e \textit{furto-supermercado}. Nestes cenários, a curva de probabilidade permaneceu saturada em níveis próximos a $1,0$ durante praticamente toda a gravação. A análise dos quadros correspondentes, exemplificada na \autoref{fig:furto-tenis1-i3d}, mostra que a simples presença do indivíduo observando ou segurando o produto já mantém a rede em estado de alerta máximo. Essa saturação evidencia a incapacidade do modelo convolucional de distinguir entre a manipulação legítima e a ocultação intencional. No caso particular de \textit{furto-supermercado}, o modelo chegou a inverter a lógica, atribuindo maior confiança de furto a seções de comportamento normal do que ao momento exato da ocultação, demonstrando na \autoref{fig:furto-supermercado-i3d}.

\begin{figure}[!htbp]
    \centering
    \caption[Curva de probabilidade do modelo I3D para o vídeo \textit{furto-tenis1}.]{Curva de probabilidade do modelo I3D para o vídeo \textit{furto-tenis1}.}
	\includegraphics[width=1.0\textwidth]{figuras/furto-tenis1-i3d.png}
	\legend{Fonte - Do autor.}
    \label{fig:furto-tenis1-i3d}
\end{figure}
\begin{figure}[!htbp]
    \centering
    \caption[Curva de probabilidade do modelo I3D para o vídeo \textit{furto-supermercado}.]{Curva de probabilidade do modelo I3D para o vídeo \textit{furto-supermercado}.}
	\includegraphics[width=1.0\textwidth]{figuras/furto-supermercado-i3d.png}
	\legend{Fonte - Do autor.}
    \label{fig:furto-supermercado-i3d}
\end{figure}

Em contrapartida, o TimeSFormer apresentou falhas mais relacionadas à latência e à interpretação equívoca do contexto global. No vídeo \textit{furto-celular}, a detecção do evento ocorreu com um atraso considerável, a observação dos quadros críticos na \autoref{fig:furto-celular-ts} mostra que a probabilidade permaneceu baixa durante o momento da subtração, superando o limiar apenas após a ação ter sido concluída. Por fim, no vídeo \textit{furto-pc}, o modelo interpretou incorretamente uma seção de comportamento normal e não reagiu durante o instante real do furto, na \autoref{fig:furto-pc-ts}. Esse comportamento indica que, em certos arranjos visuais, o mecanismo de auto-atenção global pode se fixar em elementos contextuais irrelevantes da cena, perdendo a sensibilidade para a dinâmica local da ação do indivíduo.
\begin{figure}[!htbp]
    \centering
    \caption[Curva de probabilidade do modelo TimeSFormer para o vídeo \textit{furto-celular}.]{Curva de probabilidade do modelo TimeSFormer para o vídeo \textit{furto-celular}.}
	\includegraphics[width=1.0\textwidth]{figuras/furto-celular-ts.png}
	\legend{Fonte - Do autor.}
    \label{fig:furto-celular-ts}
\end{figure}
\begin{figure}[!htbp]
    \centering
    \caption[Curva de probabilidade do modelo TimeSFormer para o vídeo \textit{furto-pc}.]{Curva de probabilidade do modelo TimeSFormer para o vídeo \textit{furto-pc}.}
	\includegraphics[width=1.0\textwidth]{figuras/furto-pc-ts.png}
	\legend{Fonte - Do autor.}
    \label{fig:furto-pc-ts}
\end{figure}

\section[Síntese dos Resultados]{Síntese dos Resultados}
A avaliação em vídeos reais contínuos revelou comportamentos distintos entre as arquiteturas ao enfrentar as condições típicas de um sistema de monitoramento. A análise preditiva aplicada aos sete vídeos reais consolida as seguintes constatações:

\begin{itemize}
\item \textbf{Seletividade e Robustez do TimeSFormer:} O modelo baseado no mecanismo de atenção demonstrou maior seletividade em suas predições. A arquitetura obteve sucesso notável ao elevar a probabilidade de anomalia de maneira sincronizada com as ações efetivas de ocultação dos itens, retornando à normalidade em seguida. As limitações encontradas neste modelo estiveram relacionadas predominantemente à latência, resultando em atraso considerável na detecção de alguns eventos. Ademais, o mecanismo de auto-atenção global mostrou-se suscetível a focar em elementos contextuais irrelevantes, perdendo a sensibilidade para as dinâmicas locais da ação do infrator.

\item \textbf{Sensibilidade a Movimentos e Saturação do I3D:} A arquitetura convolucional apresentou sucesso expressivo e alta precisão temporal na detecção de subtrações caracterizadas por movimentos abruptos. Contudo, o I3D exibiu uma clara tendência a falsos positivos e à total falta de precisão temporal na detecção de outras modalidades de furto. As curvas de probabilidade desse modelo saturaram frequentemente em patamares elevados devido à simples presença de um indivíduo manipulando os produtos, evidenciando a dificuldade da rede de distinguir entre interações legítimas e atos intencionais de ocultação.

\item \textbf{Vulnerabilidade Compartilhada a Oclusões Severas:} As duas arquiteturas falharam na detecção em cenários marcados por oclusões severas e ações corporais de baixa magnitude visual. Quando a ação era demasiadamente oclusa, os modelos eram inibidos de extrair as características espaço-temporais indispensáveis, mantendo as probabilidades de alarme significativamente abaixo do necessário para discriminação do ato.
\end{itemize}

\chapter[Avaliação de Implementação em Cenário Real]{Avaliação de Implementação em Cenário Real}
Para estruturar esta avaliação de viabilidade com rigor técnico e financeiro, a análise é decomposta em etapas fundamentais.
Inicialmente, realiza-se o levantamento do panorama financeiro atual e do histórico de perdas no varejo alimentar brasileiro, baseando-se em indicativos recentes do setor.
Sob a caracterização física de um supermercado de médio porte padronizado, executa-se o dimensionamento da rede de câmeras IP necessária para a cobertura espacial abrangente do ambiente.
Posteriormente, elabora-se a modelagem matemática da carga computacional contínua gerada pela inferência, transpondo os resultados experimentais para um dimensionamento de servidores dotados de GPUs.
Por fim, realiza-se a projeção percentual e monetária de perdas evitáveis contraposta ao custo total de aquisição e operação do hardware, obtendo a estimativa do período de recuperação do capital.

\section[Panorama de Perdas no Varejo Alimentar Brasileiro]{Panorama de Perdas no Varejo Alimentar Brasileiro}
\label{sec:estimativa_perdas}
Considerando um supermercado de médio porte com faturamento anual próximo de R\$ 90 milhões, observa-se um volume médio de perdas de aproximadamente R\$ 57.875 por mês, equivalente a cerca de R\$ 694 mil por ano, ou 0,76\% do faturamento anual \cite{aires2026}.

Embora as perdas de inventário englobem diversas categorias, como avarias, vencimentos, erros operacionais e divergências de estoque, os furtos internos e externos figuram entre as principais causas registradas pelo setor supermercadista, sendo responsáveis por aproximadamente 20\% a 35\% do total de perdas \cite{abras_perdas}.

Aplicando essa proporção ao cenário considerado, estima-se que as perdas anuais associadas a furtos situem-se entre R\$ 139 mil e R\$ 243 mil. Em termos relativos ao faturamento do estabelecimento, esse montante corresponde a aproximadamente 0,15\% a 0,27\% da receita anual.

Essa faixa de valores será utilizada nas próximas seções como referência para estimar o montante financeiro potencialmente recuperável por meio da redução de ocorrências de furto.

\section[Dimensionamento da Infraestrutura de Captação]{Dimensionamento da Infraestrutura de Captação}

Para estimar os requisitos computacionais de uma solução de detecção automática de furtos baseada em vídeo, é necessário inicialmente estabelecer uma infraestrutura de captação compatível com a cobertura integral do ambiente monitorado. Nesta análise, considera-se um supermercado de médio porte com área de vendas aproximada de 1000 m$^2$.

Diferentemente de aplicações convencionais de vigilância patrimonial, sistemas voltados à análise comportamental dependem da observação contínua das interações entre clientes e produtos ao longo dos corredores de circulação. Em ambientes supermercadistas, a presença de gôndolas e prateleiras com alturas elevadas reduz significativamente o campo de visão efetivo das câmeras e cria regiões suscetíveis a oclusões. Dessa forma, a cobertura do ambiente demanda não apenas a observação dos acessos principais, mas também o monitoramento dos corredores de venda e das áreas de maior concentração de mercadorias.

Para fins de estimativa, adota-se uma densidade de implantação correspondente a aproximadamente uma câmera para cada 15 a 20 m$^2$ de área monitorada. Essa faixa é compatível com projetos de CFTV que buscam cobertura abrangente dos corredores e sobreposição parcial entre campos de visão adjacentes.

Considerando uma área total de 1000 m$^2$, obtém-se:

\begin{equation}
N_{cam} = \frac{A}{A_{cam}}
\end{equation}

onde $A$ representa a área monitorada e $A_{cam}$ a área efetivamente coberta por cada câmera. Assumindo uma cobertura média de aproximadamente 16 m$^2$ por dispositivo, resulta:

\begin{equation}
N_{cam} = \frac{1000}{16} \approx 62,5
\end{equation}

Arredondando para um valor inteiro e considerando áreas adicionais como entradas, saídas, região de caixas e setores de maior risco operacional, adota-se como referência uma infraestrutura composta por aproximadamente 64 câmeras IP.

Uma vez estabelecida a infraestrutura de captação, o problema pode ser reinterpretado sob a perspectiva computacional. Cada uma das 64 câmeras IP instaladas produz um fluxo de vídeo contínuo e independente, potencialmente sujeito à análise automatizada por algoritmos de detecção de furtos.

Dessa forma, a carga computacional total do sistema não depende apenas das características individuais dos modelos de inteligência artificial avaliados neste trabalho, mas também da quantidade de fluxos simultâneos que devem ser processados. Em outras palavras, a infraestrutura de inferência deve ser capaz de executar continuamente o processamento de aproximadamente 64 canais de vídeo em paralelo.

Consequentemente, o dimensionamento computacional pode ser obtido a partir da capacidade de processamento por fluxo observada experimentalmente para o modelo TimeSFormer, extrapolando-se esse resultado para o conjunto completo de câmeras necessário para cobertura do ambiente monitorado. A próxima seção apresenta essa modelagem e a conversão da demanda de inferência em requisitos de hardware.

\section[Dimensionamento da Demanda de Inferência e Custos]{Dimensionamento da Demanda de Inferência e Custos}
\label{sec:estimativa_investimento}
Uma vez estabelecida a necessidade de monitoramento de aproximadamente 64 câmeras IP para cobertura integral de um supermercado de médio porte, torna-se possível converter a demanda operacional do sistema em requisitos concretos de processamento computacional. Conforme discutido na \autoref{sec:test_performance}, o melhor modelo obtido neste trabalho foi o TimeSFormer pré-treinado no banco de dados SSv2 com entrada de 32 quadros e ajuste completo dos parâmetros, alcançando o maior F2-Score no conjunto de teste, considerando que o objetivo desta análise consiste em dimensionar a infraestrutura necessária para o melhor modelo obtido, adota-se esse TimeSFormer como referência.

Os experimentos de eficiência, na \autoref{sec:efficiency}, demonstraram que o TimeSFormer executa aproximadamente 1,35 inferências por segundo em uma GPU GTX 1660 Super, correspondendo a um tempo médio de 738,53 ms para processar uma única janela de vídeo composta por 32 quadros.

A GTX 1660 Super utilizada nos experimentos possui desempenho teórico próximo de 5 TFLOPS em operações FP32. Dessa forma, pode-se estimar a demanda computacional associada a uma única inferência do modelo através da relação entre o desempenho disponível e o tempo efetivamente consumido durante a execução:

\begin{equation}
C_{inf}=P \cdot t
\end{equation}

onde $P$ representa a capacidade computacional da GPU (em TFLOPS) e $t$ o tempo médio de inferência (em segundos).

Substituindo os valores experimentais:

\begin{equation}
C_{inf}=5 \times 0,73853
\end{equation}

\begin{equation}
C_{inf}\approx 3,69\ \text{TFLOP}
\end{equation}

Portanto, cada janela processada pelo TimeSFormer demanda aproximadamente 3,69 TFLOP de processamento.

Assumindo o cenário conservador em que as 64 câmeras instaladas estejam simultaneamente produzindo atividade relevante e necessitem ser analisadas continuamente, a carga computacional total torna-se:

\begin{equation}
C_{total}=64 \times 3,69
\end{equation}

\begin{equation}
C_{total}\approx 236 \text{ TFLOP}
\end{equation}

Este valor representa a capacidade computacional necessária para sustentar o processamento simultâneo dos fluxos de vídeo no mesmo intervalo temporal observado experimentalmente.

Atualmente, aceleradores de inferência voltados à inteligência artificial disponíveis comercialmente apresentam desempenho significativamente superior ao hardware utilizado durante os experimentos. Como referência, uma GPU NVIDIA RTX 5090 disponibiliza aproximadamente 105 TFLOPS em precisão FP32, enquanto uma RTX 5080 disponibiliza cerca de 56 TFLOPS.

Dessa forma, o processamento completo da loja poderia ser atendido por diferentes configurações equivalentes de hardware, tais como:

\begin{itemize}
\item 3 GPUs RTX 5090 ($3 \times 105 \approx 315$ TFLOPS);
\item 5 GPUs RTX 5080 ($5 \times 56 \approx 280$ TFLOPS);
\item infraestrutura distribuída composta por múltiplos servidores menores.
\end{itemize}

Sob a perspectiva econômica, considerando valores praticados no mercado brasileiro durante 2026, uma RTX 5090 possui custo aproximado entre R\$ 18.000 e R\$ 22.000. Assim, uma infraestrutura baseada em três placas totalizaria investimento próximo de R\$ 60.000, desconsiderando os demais componentes do servidor.

Incluindo processador, memória, armazenamento, sistema de alimentação redundante e margem para expansão futura, o custo total de implantação da infraestrutura de inferência situa-se tipicamente entre R\$ 70.000 e R\$ 90.000 para uma unidade varejista de aproximadamente 1000 m\textsuperscript{2}.

É importante destacar que esta estimativa representa um cenário deliberadamente conservador, no qual todas as câmeras são processadas continuamente e de forma independente. Na prática, mecanismos de otimização podem reduzir substancialmente a demanda computacional. Entre eles destacam-se o processamento em lote, a redução da frequência de inferência em áreas de baixo risco, a utilização de detectores de movimento para ativação seletiva do modelo e a execução distribuída em múltiplos períodos temporais. Consequentemente, o custo real de implantação tende a ser inferior ao valor estimado nesta análise.

\section[Estimativa de Redução de Perdas e Retorno Sobre o Investimento]{Estimativa de Redução de Perdas e Retorno Sobre o Investimento}

A etapa final desta análise consiste em converter os resultados de desempenho obtidos experimentalmente em uma estimativa econômica para um cenário real de operação.

Conforme discutido no \autoref{chap:introducao}, estudos sobre monitoramento por CFTV indicam que operadores humanos podem falhar na identificação de até 66\% dos eventos críticos presentes em seu campo de visão. Em outras palavras, mesmo em ambientes monitorados, apenas aproximadamente 34\% das ocorrências potencialmente observáveis tendem a ser efetivamente identificadas em tempo real.

Por sua vez, o melhor modelo obtido neste trabalho, o TimeSFormer pré-treinado no banco de dados SSv2 com entrada de 32 quadros e ajuste completo dos parâmetros, alcançou revocação de 91,7\% no conjunto de teste. Como a revocação representa a proporção de furtos corretamente identificados pelo sistema, este valor pode ser interpretado como a capacidade potencial de detecção automática do modelo.

Assumindo que a taxa de falha humana e a taxa de revocação do modelo sejam representativas do ambiente operacional, a diferença entre ambos corresponde ao percentual adicional de ocorrências passíveis de identificação:
...
Assim, o sistema proposto possui potencial para identificar 57,7\% mais ocorrências de furto do que o monitoramento exclusivamente humano.

Na \autoref{sec:estimativa_perdas}, foi estimado que um supermercado de médio porte apresenta perdas anuais associadas a furtos entre R\$139 mil e R\$243 mil. Considerando que a redução de perdas seja proporcional ao aumento da capacidade de detecção, obtém-se uma estimativa conservadora do valor recuperável:

\begin{equation}
G_{min}=0,577 \times 139.000
\end{equation}

\begin{equation}
G_{min}\approx R\$80.200/ano
\end{equation}

e

\begin{equation}
G_{max}=0,577 \times 243.000
\end{equation}

\begin{equation}
G_{max}\approx R\$140.200/ano
\end{equation}

Portanto, a implantação do sistema possui potencial para reduzir perdas financeiras em aproximadamente R\$80 mil a R\$140 mil por ano para uma unidade varejista de porte médio.

Na \autoref{sec:estimativa_investimento}, estimou-se um investimento de implantação entre R\$70 mil e R\$90 mil para a infraestrutura de inferência necessária ao processamento das câmeras do estabelecimento. Dessa forma, o período de recuperação do capital investido pode ser estimado por:
...
\begin{equation}
Payback_{conservador}
=
\frac{90.000}{80.200}
\approx 1,12 \text{ anos}
\end{equation}
...
=
\frac{70.000}{140.200}
\approx 0,50 \text{ anos}
\end{equation}

Portanto, mesmo considerando hipóteses conservadoras para a infraestrutura computacional e para a efetividade operacional do sistema, observa-se que o investimento apresenta potencial de retorno em um horizonte entre \textbf{seis} e \textbf{quatorze meses}. Além do benefício financeiro direto associado à redução de perdas, a solução proposta ainda oferece vantagens adicionais não quantificadas nesta análise, como aumento da capacidade de monitoramento, padronização da vigilância, redução da fadiga operacional e geração automática de alertas em tempo real para as equipes de prevenção de perdas.

\chapter[Conclusões]{Conclusões}

O presente trabalho abordou um problema crônico para o setor varejista global: a necessidade de identificação automatizada de comportamentos suspeitos e furtos, reduzindo a dependência quase exclusiva e limitante da vigilância humana em CFTVs. Neste contexto, o objetivo principal da pesquisa foi executado comparando empiricamente o desempenho de duas arquiteturas distintas de aprendizado profundo na tarefa de modelagem e detecção de anomalias espaço-temporais em vídeos: a RNC 3D (I3D) e o Transformador de Visão (TimeSFormer). Para viabilizar este estudo, o trabalho apresentou a contribuição metodológica de curadoria e de construção de um banco de dados próprio, agregando, filtrando e segmentando amostras das bases públicas DCSASS, Shoplifting Dataset 2.0 e Shoplifting MNNIT.

Através da formulação de dezesseis configurações experimentais, a pesquisa avaliou o impacto de diferentes estratégias de ajuste de pesos, uso de múltiplas modalidades de entrada e diferentes resoluções temporais. A síntese dos resultados obtidos durante o treinamento respondeu ao problema de pesquisa, constatando-se primeiramente que o ajuste profundo de todos os pesos da rede é estritamente essencial para a convergência e o aprendizado eficaz das dinâmicas de furto em ambas as arquiteturas em bancos de dados reduzidos como o construído. Experimentos que limitaram o treinamento apenas à camada classificadora final falharam expressivamente em adaptar as representações visuais ao novo domínio, estacionando em altos patamares de erro mesmo após dezenas de épocas de treinamento. Além disso, demonstrou-se que a inclusão de imagens de fluxo óptico no modelo I3D atua como um mecanismo compensatório relevante apenas quando o extrator de características original é mantido congelado, tornando-se redundante quando a RNC é ajustada por completo.  A avaliação definitiva sobre o conjunto de teste confirmou a superioridade dos mecanismos de atenção global para a compreensão semântica da cena, evidenciando uma importante inversão no ranqueamento de desempenho em relação à fase de validação. A arquitetura TimeSFormer destacou-se por sua notável capacidade de generalização para dados inéditos, alcançando 100\% de precisão, o que denota um controle superior na redução de alarmes falsos, além de obter o melhor F2-Score geral (0,932). Por sua vez, o modelo convolucional I3D exibiu um comportamento preditivo marcadamente conservador, com uma precisão consideravelmente menor (72,3\%), refletindo uma forte tendência em classificar interações normais como ocorrências anômalas.

Essa diferença de comportamentos foi reiterada e aprofundada na etapa de inferência baseada em janela deslizante, que simulou o monitoramento de vídeos reais e contínuos. Nesse cenário extremo, o TimeSFormer provou ser mais seletivo, obtendo êxito ao elevar as probabilidades de anomalia de maneira sincronizada ao exato instante da ocultação dos produtos. Em contrapartida, embora o I3D tenha confirmado sua capacidade de detectar ações abruptas com precisão temporal, o modelo falhou criticamente ao apresentar saturações e falsos positivos contínuos gerados por simples manipulações legítimas de itens, evidenciando a dificuldade da arquitetura convolucional em distinguir a observação normal da intenção ilícita.

Sob a ótica da aplicação mercadológica, o trabalho mapeia o compromisso técnico necessário para a implementação desses sistemas na indústria varejista. Verificou-se que a obtenção da precisão adicional e da seletividade proporcionada pelo TimeSFormer exige um investimento computacional substancialmente maior, com tempo de inferência de 738,53 milissegundos por janela. Já o I3D destaca-se pela alta eficiência e baixa latência (79,52 milissegundos), configurando-se como uma alternativa viável e escalável para ambientes com severas restrições de infraestrutura, desde que sua sensibilidade aos falsos positivos possa ser contornada por lógicas de limiares mais estritas. 

Apesar dos resultados, a pesquisa identifica limitações inerentes aos modelos avaliados. Ambas as arquiteturas compartilham uma vulnerabilidade crítica: a falha completa de detecção diante de oclusões severas na cena (causadas por estantes ou aglomerações) e de movimentos corporais propositalmente sutis. Nestas condições, a ocultação dos membros ou do objeto inibe fortemente a extração das características espaço-temporais essenciais, fazendo com que as probabilidades de comportamento suspeito permaneçam abaixo do limiar de alarme necessário para a discriminação do ato. Além disso, o TimeSFormer demonstrou alguns episódios de latência no alarme e, sob certos arranjos visuais, uma tendência a fixar seu mecanismo de atenção global em elementos contextuais irrelevantes, perdendo sensibilidade para as ações suspeitas.

Como sugestões para estudos futuros, recomenda-se a expansão do banco de dados com filmagens locais que capturem a grande diversidade de iluminação, densidade de clientes e arquiteturas de prateleiras características do varejo brasileiro. Na área experimental, sugere-se a realização de investigações voltadas à otimização fina do treinamento por meio da busca sistemática por melhores hiperparâmetros, com foco especial na calibração da taxa de aprendizado e no seu método de decaimento. Para conferir maior robustez estatística aos resultados e garantir a estabilidade da convergência, recomenda-se a implementação de estratégias de validação cruzada (K-fold) ou métodos de busca em grade, permitindo uma exploração mais granular do espaço de parâmetros. Em termos arquiteturais, a incorporação de abordagens multimodais, em especial a integração com Redes Convolucionais baseadas em estimativa de pose esqueletal, representa um caminho promissor para manter a coerência do rastreamento corporal mesmo quando parte do indivíduo está oclusa.
...
\label{apendice:github}
% ----------------------------------------------------------

O código-fonte, os experimentos e a documentação desenvolvidos neste trabalho encontram-se publicamente disponíveis no GitHub, organizados em três repositórios distintos, cada um responsável por um escopo bem delimitado do projeto.

\section*{Organização dos Repositórios}

\begin{itemize}
    \item \textbf{\href{https://github.com/luis-cmenezes/shoplifting-detection}{shoplifting-detection}}: \textit{monorepo} principal do projeto. Contém os \textit{scripts} centralizados de download e pré-processamento do banco de dados, os \textit{pipelines} de treinamento e validação de ambas as arquiteturas, as configurações dos experimentos via \texttt{config.yaml} e os artefatos gerados (pesos dos modelos e registros de métricas). Inclui os subpacotes \texttt{i3d-shoplifting} e \texttt{timesformer-shoplifting} como dependências locais.

    \item \textbf{\href{https://github.com/luis-cmenezes/i3d-shoplifting}{i3d-shoplifting}}: subpacote responsável pelo \textit{fine-tuning} do modelo I3D como classificador binário. Implementa a arquitetura I3D em PyTorch puro, os módulos de pré-processamento para extração de quadros RGB e geração de fluxo óptico TV-L1, e o laço de treinamento com \texttt{BCEWithLogitsLoss} e ponderação de classes.

    \item \textbf{\href{https://github.com/luis-cmenezes/timesformer-shoplifting}{timesformer-shoplifting}}: subpacote responsável pelo \textit{fine-tuning} do modelo TimeSFormer como classificador binário. Implementa o \textit{dataset} PyTorch com amostragem temporal via Decord, os utilitários de padronização de vídeo com FFmpeg e o treinamento por meio do \texttt{Trainer} da biblioteca \textit{Transformers} com \texttt{CEL} e ponderação de classes.
\end{itemize}

\section*{Gerenciamento de Dependências com UV}

Todos os repositórios adotam a ferramenta \textbf{UV} para o gerenciamento do ambiente Python. O UV é um gerenciador de pacotes e ambientes virtuais de alto desempenho que resolve e instala dependências de forma significativamente mais rápida que as ferramentas tradicionais. A configuração completa do ambiente de cada repositório é declarada no arquivo \texttt{pyproject.toml}, permitindo a reprodução integral do ambiente de desenvolvimento com um único comando.
